...
class VacaExcelReport(object):
	...
	def get_content(self):
		...
		sheet = wb.add_worksheet('Prov. Vacaciones')

		row = 8
		sheet.autofilter('A'+str(row)+':Z'+str(row))
		...
		sheet.set_column(12,24+len_prom_columns, 15)
		...
		sheet.write(row - 1,9,"CUENTA ANALITICA",column_content_table_format)
		...
		# sheet.write(row - 1,23+len_prom_columns,"DIAS NO COMPUTABLES",column_content_table_format)
		sheet.write(row - 1,20+len_prom_columns,"DIAS GANADOS",column_content_table_format)
		# sheet.merge_range(row - 2, 22+len_prom_columns,row - 2, 23+len_prom_columns, 'PROVISION', column_content_table_format_top)
		sheet.write(row - 1,21+len_prom_columns,"PROVISIÓN ACUMULADA",column_content_table_format)
		sheet.write(row - 1,22+len_prom_columns,"PROVISIÓN MES",column_content_table_format)
  # sheet.merge_range(row - 2, 24+len_prom_columns,row - 2, 25+len_prom_columns, 'DESCUENTOS', column_content_table_format_top)
		sheet.write(row - 1,23+len_prom_columns,"LBS",column_content_table_format)
		...
		for i,dict in enumerate(self.data):
			...
			# sheet.write(i + row,23+len_prom_columns,dict["Dias no computables"],style_content_num_table_format)
			sheet.write(i + row,20+len_prom_columns,dict["Dias ganados"],style_content_num_table_format)
			sheet.write(i + row,21+len_prom_columns,dict["Provision Acum"],style_content_num_table_format)
			sheet.write(i + row,22+len_prom_columns,dict["Provision Actual"],style_content_num_table_format)
			sheet.write(i + row,23+len_prom_columns,dict["lbs"],style_content_num_table_format) ,
		...
		for i in range(16,24+len_prom_columns):
