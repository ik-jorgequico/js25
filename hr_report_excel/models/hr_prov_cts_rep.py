...
class CTSExcelReport(object):
	...
	def get_content(self):
		...
		sheet = wb.add_worksheet('Prov. CTS')

		row = 8
		sheet.autofilter('A1'+str(row)+':AK'+str(row))
		# sheet.autofilter('A1:AC'+ str(row))
		...
		sheet.set_column(12,27+len_prom_columns, 15)
		...
		sheet.write(row - 1,13,"RÉGIMEN",column_content_table_format)
		sheet.write(row - 1,14,"F INGRESO",column_chiki)
		sheet.write(row - 1,15,"F CESE",column_chiki)

		# sheet.merge_range(row - 2,15,row - 2,19+len_prom_columns, 'BASE CALCULO', column_content_table_format_top)

		sheet.write(row - 1,16,"BÁSICO",column_content_table_format)
		sheet.write(row - 1,17,"ASIG FAMILIAR",column_content_table_format)
		sheet.write(row - 1,18,"1/6 GRATI",column_content_table_format)
		...
		# sheet.write(row - 1,27+len_prom_columns,"TOTAL DESCUENTO",column_content_table_format)
		# sheet.write(row - 1,28+len_prom_columns,"NETO A PAGAR",column_content_table_format)
		...
		for i,dict in enumerate(self.data):
			...
			sheet.write(i + row,18,dict["1/6 Grati"],style_content_num_table_format)
			...
			sheet.write(i + row,24+len_prom_columns,dict["Provision Acum"],style_content_num_table_format)
			sheet.write(i + row,25+len_prom_columns,dict["Provision Actual"],style_content_num_table_format)
			sheet.write(i + row,26+len_prom_columns,dict["lbs"],style_content_num_table_format)
			# sheet.write(i + row,26+len_prom_columns,dict["Neto a Pagar"],style_content_num_table_format)
		...
		for i in range(16,27+len_prom_columns):
