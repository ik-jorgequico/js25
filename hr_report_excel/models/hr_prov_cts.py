...
from .hr_prov_cts_rep import CTSExcelReport
import base64

class BonCts(models.Model):
    _name = 'hr.prov.cts'
    _inherit = ['portal.mixin', 'mail.thread', 'mail.activity.mixin']
    _description = 'Provision CTS'
    ...
    period_cts = fields.Char(string="Nombre Periodo",tracking=True,store=True)
    ...
    child_ids = fields.One2many("hr.prov.cts.line","parent_id",string="Provisiones CTS", states={'refuse': [('readonly', True)], 'cancel': [('readonly', True)], 'approve': [('readonly', True)]})
    ...
    def _compute_name(self):		
        if self.date_from and self.date_to :
            ...
            self.name = "Provisión CTS - " + name_period + " - " + year_period
    ...
    def _compute_dates(self):
        if self.anio and self.period:
            ...
            if self.period in ['11','12']:
                _ranio = int(self.anio)
                _sanio = int(self.anio) + 1
                self.period_cts = "NOVIEMBRE " + str(_ranio) + " - ABRIL " + str(_sanio)
    
            elif self.period in ['01','02','03','04']:
                _sanio = int(self.anio)-1
                self.period_cts = "NOVIEMBRE " + str(_sanio) + " - ABRIL " + str(self.anio)
    
            else:
                _sanio = int(self.anio)
                self.period_cts = "MAYO " + str(self.anio) + " - OCTUBRE " + str(_sanio)
    ...
    def compute_sheet(self):
        ...
        if payslips:
            ...
            for employee in employees:
                ...
                cts_prev , days_prev_ant, days_total_ant = self._cts_prev(employee, self.date_from)
                cts_lbs_value, cts_pagada_value = self._cts_lbs(employee, self.date_from, self.date_to)
                ...
                val_list.append({
                    ...
                    "total_prev": cts_prev if employee.contract_id.peru_employee_regime.abbr == 'RM' else 0 ,
                    "days_prev": days_prev_ant + days_total_ant,
                    "cts_lbs": cts_lbs_value,
                    "cts_pagada": cts_pagada_value,
                    "analytic_ac": account_analytic_active,
                })
            
            self.env["hr.prov.cts.line"].create(val_list)            
            return  self.env.cr.commit()
        self.action_submit()
            

    def _cts_lbs(self, employee, date_from, date_to):
        cont = 0
        cts_pagada = 0
        ...
        cts = self.env["hr.cts.line"].search([
            ("date_to","=",date_to),
            ("employee_id","=",employee.id),
        ])
        ...
        if lbs.cts_amount:
            ...
            cts_pagada = cts.total
            
        return cont, cts_pagada
    ...
    def _get_gratification(self, employee, payslips, date_from, date_to):
            if int(date_from.strftime("%m")) < 7:
                year = int(date_from.strftime("%Y")) - 1
                date_from =  datetime.strptime("01/12/"+str(year), '%d/%m/%Y')
                date_to = datetime.strptime("31/12/"+str(year), '%d/%m/%Y')
            else:
                date_from = datetime.strptime("01/07/"+date_from.strftime("%Y"), '%d/%m/%Y')
                date_to = datetime.strptime("31/07/"+date_from.strftime("%Y"), '%d/%m/%Y')
            
            grati = payslips.search([("date_from",">=",date_from),
                                    ("date_to","<=",date_to),
                                    ("employee_id.id","=",employee.id)], limit = 1)
            
            for line in grati.line_ids.filtered(lambda input: input.code == "GRATI"):
    ...
    def _compute_sublines(self, date_from, employee):
        ...
        per_1 = ['11','12','01','02','03','04']
        per_2 = ['05','06','07','08','09','10']

        m_f = date_from.strftime("%m")
        ...
        if m_f in per_1:
            ...
            new_date =  datetime.strptime("01/11/"+str(year), '%d/%m/%Y')
            ...
            new_date =  datetime.strptime("01/05/"+ year, '%d/%m/%Y')
        ...
        for obj in self:
            ...
            for child_id in obj.child_ids:
                val1 = {
                    ...
                    # "n cuenta": child_id.employee_id.bank_cts_id.acc_number or '',
                ...
                val3 = {
                    ...
                    "1/6 Grati":child_id.gratification,
                    ...
                    "Provision Actual":child_id.total_prov_mes,#PROVISION ACTUAL
                    "Provision Acum":child_id.total_prev, #PROVISION ACUMULADA
                    "Provision Mes":child_id.prov_mes_adjust,#PROVISION MES
                    "lbs":child_id.cts_lbs,
                    # "Total Descuento":0,
                    # "Neto a Pagar":child_id.total,
    ...
    report_id = fields.Many2one('ir.actions.report',
        string="Report", domain="[('model','=','hr.prov.cts.line'),('report_type','=','qweb-pdf')]", default=_get_default_report_id)
    ...
    def _unlink_if_draft_or_cancel(self):
        if any(cts.state not in ('draft', 'cancel') for cts in self):
            raise UserError(_('You cannot delete a cts which is not draft or cancelled!'))

    def _cts_prev(self,employee,date_from):
        cts_prev = 0
        days_prev_ant = 0
        days_total_ant = 0

        if date_from.strftime("%m") == '05' or  date_from.strftime("%m") == '11':
            cts_prev = 0
            days_prev_ant = 0
            days_total_ant = 0
            ...
            if date_from.strftime("%m") == '01':
                y_pf = int(date_from.strftime("%Y")) - 1
                new_date_from = datetime.strptime("01/12/"+ str(y_pf), '%d/%m/%Y')
                data = self.env["hr.prov.cts.line"].search([("date_from","=",new_date_from),
                                            ("employee_id","=",employee.id)])				
                cts_prev = data.total_prov_mes
                ...
                days_prev_ant = data.days_prev
                days_total_ant = data.number_total
        return cts_prev , days_prev_ant, days_total_ant


class BonCtsLine(models.Model):
    _name = 'hr.prov.cts.line'
    _description = 'Provision CTS Line'
    ...
    parent_id = fields.Many2one("hr.prov.cts",string="CTS", ondelete='cascade', store=True,)
    subline_ids = fields.One2many("hr.prov.cts.subline","cts_line",string="Variables")
    ...
    cts_lbs = fields.Float(string='LBS', default= 0, store=True)
    
    cts_pagada = fields.Float(string='cts_pagada', default= 0, store=True,)
    ...
    @api.depends('cts_lbs', 'total_prev', 'total')
    def _compute_prov_mes_adjust(self):
        for record in self:
            if record.cts_pagada > 0:
                record.prov_mes_adjust = record.cts_pagada - record.total_prev
            elif record.cts_lbs > 0:
                record.prov_mes_adjust = record.cts_lbs - record.total_prev
            else:
                record.prov_mes_adjust = record.total
        
    @api.depends('employee_id.contract_id.structure_type_id', 'total', 'total_prev')
    def _compute_total_prov_mes(self):
        for record in self:
            ...
            if abbr == 'RG':
                ...
                record.total_prov_mes = (record.prov_mes_adjust + record.total_prev)/2
            elif abbr == 'RM':
                record.total_prov_mes = 0
    ...
    def _compute_total(self):
        for record in self:
            record.total = round(record.total_amount/360*((record.number_total + record.days_prev)) - record.total_prev, 2)
...
class BonCtsSubLine(models.Model):
    _name = 'hr.prov.cts.subline'
    _description = 'Provision CTS Subline'

    name = fields.Char(string="Nombre Concepto")
    cont = fields.Float(string="Conteo Meses")
    amount = fields.Float(string="Monto")
    average = fields.Float(string="Promedio")
    cts_line = fields.Many2one("hr.prov.cts.line", ondelete='cascade', store=True)