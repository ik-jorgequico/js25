...
class HrLbsReport(Controller):
    ...
    @route(["/print/lbs_cts"], type='http', auth='user')
    def get_lbs_cts_report_print(self, list_ids='', **post):
        ...
        if lbs_lines:
            ...
            for lbs_line in lbs_lines:
                ...
                if not lbs_line.parent_id or not lbs_line.parent_id.report_cts_id:
                    report = request.env.ref('hr_lbs_reports.action_report_cts_lbs', False)

                else:
                    report = lbs_line.parent_id.report_cts_id
                ...
                report_name =  "Constancia de cts - " + lbs_line.employee_id.name
                if lbs_line.parent_id.report_cts_id.print_report_name:
                    report_name = safe_eval(lbs_line.parent_id.report_cts_id.print_report_name, 
                                            {'object': lbs_line})
