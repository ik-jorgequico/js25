...
class HrLbsReport(Controller):

    @route(["/print/lbs_work"], type='http', auth='user')
    def get_lbs_work_report_print(self, list_ids='', **post):
        ...
        if lbs_lines:
            ...
            for lbs_line in lbs_lines:
                ...
                if not lbs_line.parent_id or not lbs_line.parent_id.report_work_id:
                    report = request.env.ref('hr_lbs_reports.action_report_work_lbs', False)
                else:
                    report = lbs_line.parent_id.report_work_id
                ...
                report_name =  "Constancia de trabajo - " + lbs_line.employee_id.name
                if lbs_line.parent_id.report_work_id.print_report_name:
                    report_name = safe_eval(lbs_line.parent_id.report_work_id.print_report_name, 
                                            {'object': lbs_line})
