...
class HrContract(models.Model):
    ...
    contract_file = fields.Many2one('ir.attachment', string='Contrato PDF', store=True)
    ...
    def send_contract_email(self):
        ...
        try:
            if(self.previous_contract):
                ...
                # INICIAMOS PARA LA SELECCION DEL CONTRATO DE A&R
                # ('COORDINADOR','COORDINADOR'),
                # ('SUPERVISOR','SUPERVISOR'),
                # ('VENDEDOR - MASIVO','VENDEDOR - MASIVO'),
                # ('ASESOR DE VENTAS','ASESOR DE VENTAS'),
    ...
    def get_float_to_spanish_letters(self,number):
        def float_to_spanish_letters(number):
            units = {
            0: 'CERO',
            1: 'UNO',
            2: 'DOS',
    ...
    #             if billions > 1:
    #                 result += float_to_spanish_letters(billions) + " MIL"
    #             else:
    #                 result += "MIL"
    #             if integer_part > 0:
    #                 result += " "
    ...
    #             if millions > 1:
    #                 result += float_to_spanish_letters(millions) + " MILLONES"
    #             else:
    #                 result += "UN MILLON"
    #             if integer_part > 0:
    #                 result += " "
    ...
    #             integer_part %= 1000
    #             if thousands > 1:
    #                 result += float_to_spanish_letters(thousands) + " MIL"
    #             else:
    #                 result += "MIL"
    #             if integer_part > 0:
    #                 result += " "
    ...
    #     result = float_to_spanish_letters(number)
    ...
    def compute_contract_format(self):
        #Si la empresa actual tiene el vat 
        # ruc = {
        #     20610103023: 'A & R OUTSOURCING Y GESTION E.I.R.L.',#ID 1
        #     20546854834: 'NAZCA TEX S.A.C.',                    #ID 2
        #     20429055696: 'J.S.MARKET SOLUTIONS S.A.C.',         #ID 3
        # }
        if(self.previous_contract):
            # report_template_id = self.env.ref('hr_contrac_format.report_renovation_report' ).with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
            ...
            if self.work_type == 'ADM':
                if(self.labor == 'Labor hibrida'):
                    # report_template_id = self.env.ref('hr_contrac_format.report_ayr_vendor_mass' ).with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
                    report_template_id = self.env['ir.actions.report']._render_qweb_pdf('hr_contrac_format.report_ayr_vendor_mass', self.id)
                elif(self.labor == 'Labor teletrabajo'):
                    # report_template_id = self.env.ref('hr_contrac_format.report_adm_teletrabajo' ).with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
                ...
                # report_template_id = self.env.ref('hr_contrac_format.report_operative_operative_report' ).with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
                ...
                # report_template_id = self.env.ref('hr_contrac_format.report_operative_seller_report' ).with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
            ...
            # ('PART TIME','PART TIME'),
                ...
                # report_template_id = self.env.ref('hr_contrac_format.report_ayr_coordinador').with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
                report_template_id = self.env['ir.actions.report']._render_qweb_pdf('hr_contrac_format.report_ayr_coordinador', self.id)

            elif self.work_type == 'SUPERVISOR':
                # report_template_id = self.env.ref('hr_contrac_format.report_ayr_supervisor_ventas').with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
                ...
                # report_template_id = self.env.ref('hr_contrac_format.report_ayr_asesor_ventas').with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
                report_template_id = self.env['ir.actions.report']._render_qweb_pdf('hr_contrac_format.report_ayr_asesor_ventas', self.id)
            
            elif self.work_type == 'PART TIME':
                # report_template_id = self.env.ref('hr_contrac_format.report_ayr_part_time').with_context(force_report_rendering=True)._render_qweb_pdf(self.id)
        ...
        ir_values = {
            'name': self.pdf_filename+'.pdf',
            'type': 'binary',
            'datas': record,
            'store_fname': record,
            'mimetype': 'application/x-pdf',
        }
        self.contract_file = self.env['ir.attachment'].create(ir_values)
